      ...
      if (isEditMode && course) {
        ...
        const updateData: CourseUpdateRequest = {
          ...
          videoUrl: useVideoUpload 
            ? (videoFiles.length > 0 ? "uploaded-video-url" : course.videoUrl)
            : (data.videoUrl || course.videoUrl),
          thumbnailUrl: useThumbnailUpload 
            ? (thumbnailFiles.length > 0 ? "uploaded-thumbnail-url" : course.thumbnailUrl)
            : (data.thumbnailUrl || course.thumbnailUrl),
        };

        await unifiedCourseService.updateCourseMock(course.id, updateData);
        ...
        const createData: CourseCreateRequest = {
          ...data,
          videoUrl: useVideoUpload 
            ? (videoFiles.length > 0 ? "uploaded-video-url" : "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4")
            : (data.videoUrl || "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4"),
          thumbnailUrl: useThumbnailUpload 
            ? (thumbnailFiles.length > 0 ? "uploaded-thumbnail-url" : "/assets/placeholder-image.jpeg")
            : (data.thumbnailUrl || "/assets/placeholder-image.jpeg"),
        };

        await unifiedCourseService.createCourseMock(createData);
