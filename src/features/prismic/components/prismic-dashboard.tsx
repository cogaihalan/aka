...
import { Plus, ExternalLink, FileText, Edit } from "lucide-react";
import { formatDistanceToNow } from "date-fns";

import { prismicApiService } from "@/lib/api/prismic-service";
import { PrismicPage } from "@/types/prismic";

export function PrismicDashboard({}) {
  ...
  const getLastUpdated = () => {
    ...
    return formatDistanceToNow(new Date(recentPages[0].last_publication_date), {
      addSuffix: true,
    });
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Latest content updates from Prismic</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              >
                <div className="flex-1">
                  <h4 className="font-medium">
                    {page.data?.title || page.data?.name || "Untitled"}
                  </h4>
                  <p className="text-sm text-muted-foreground">
                    /{page.uid} •{" "}
                    {formatDistanceToNow(new Date(page.last_publication_date), {
                      addSuffix: true,
                    })}
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      window.open(
                        `${process.env.NEXT_PUBLIC_PRISMIC_URL}/documents/${page.id}`,
                        "_blank"
                      )
                    }
                  >
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Button
              onClick={() =>
                window.open(
                  `${process.env.NEXT_PUBLIC_PRISMIC_URL}/documents/new`,
                  "_blank"
                )
              }
            ...
            <Button
              ...
              onClick={() =>
                window.open(
                  process.env.NEXT_PUBLIC_PRISMIC_URL || "https://prismic.io",
                  "_blank"
                )
              }
              className="h-20 flex-col gap-2"
