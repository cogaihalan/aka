...
import { NavigationState, NavigationFilters, FilterCounts } from '@/types/navigation';
...
export function useNavigation(options: UseNavigationOptions = {}) {
  ...
  const updateUrl = useCallback((newState: NavigationState) => {
    ...
    // Add price range
    if (newState.filters.priceRange && 
  ...
  const updateFilters = useCallback((newFilters: Partial<NavigationFilters>) => {
    setState(prevState => {
  ...
  useEffect(() => {
    ...
    setState(prevState => {
      ...
      const filtersChanged = JSON.stringify(prevState.filters) !== JSON.stringify(newState.filters);
