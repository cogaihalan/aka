...
import { PrismicPage, PrismicStaticPage, PrismicBlogPost } from "@/types/prismic";
    ...
    if (page) {
      return {
        ...
        openGraph: {
          ...
          description: page.data?.meta_description || '',
          images: page.data?.featured_image?.url ? [page.data.featured_image.url] : [],
        },
      };
    }
...
export default async function DynamicPage({ params }: DynamicPageProps) {
  ...
  try {
    ...
    if (staticPage) {
      return (
        <Suspense fallback={<PrismicPageSkeleton />}>
          <PrismicPageRenderer content={staticPage} type="static_page" />
        </Suspense>
      );
    }
