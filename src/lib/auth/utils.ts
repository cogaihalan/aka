 ...
 */
export function isValidPermission(permission: string): permission is UserPermission {
 ...
 */
export function createDefaultUserMetadata(role: UserRole = UserRole.USER): ClerkUserMetadata {
  return {
    role,
    isActive: true,
...
export function updateUserMetadata(
  ...
  const role = updates.role || (currentMetadata.role as UserRole) || UserRole.USER;
  const permissions = ROLE_PERMISSIONS[role];
  const isActive = updates.isActive !== undefined ? updates.isActive : (currentMetadata.isActive !== false);
 ...
 * Create auth error
 */
export function createAuthError(message: string, code: string = "AUTH_ERROR"): AuthError {
 ...
 */
export function createPermissionError(permission: UserPermission): PermissionError {
...
export function getRouteProtection(pathname: string): {
  ...
  if (pathname.startsWith("/dashboard")) {
    return {
      ...
      allowedPermissions: [UserPermission.ACCESS_DASHBOARD],
  ...
  if (pathname.startsWith("/account")) {
    return {
      requireAuth: true,
      allowedPermissions: [UserPermission.MANAGE_PROFILE],
      redirectTo: "/auth/sign-in",
    };
  }

  // Storefront routes - all users (including guests)
