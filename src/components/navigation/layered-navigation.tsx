...
export function LayeredNavigation({
  ...
  const handleArrayFilterChange = useCallback((filterKey: string, optionId: string, checked: boolean) => {
  ...
  const getFilterCount = useCallback((groupId: string, optionId: string) => {
    ...
    return (
      <Collapsible key={group.id} open={isExpanded} onOpenChange={() => toggleGroup(group.id)}>
        <CollapsibleTrigger asChild>
          <Button
            variant="ghost"
            className={cn("w-full justify-between p-2 h-auto font-medium text-sm")}
          >
        ...
        <CollapsibleContent className={cn("space-y-3 mt-3 transition-all duration-300 ease-out")}>
          {group.type === 'range' && (
          ...
          {group.type === 'checkbox' && (
            <div className={cn("space-y-2")}>
              {group.options.map((option) => {
                const isChecked = (filters[group.id] as string[])?.includes(option.value) || false;
                const count = getFilterCount(group.id, option.value);
                
                return (
                  <div key={option.id} className={cn("flex items-center space-x-2 transition-all duration-200 hover:bg-muted/50 rounded-md p-1 -m-1")}>
                    <Checkbox
                      id={`${group.id}-${option.id}`}
                      checked={isChecked}
                      onCheckedChange={(checked) =>
                        handleArrayFilterChange(group.id, option.value, checked as boolean)
                      }
                      className="transition-all duration-200"
                    />
                    <Label
                      htmlFor={`${group.id}-${option.id}`}
                      className={cn("flex-1 text-sm cursor-pointer flex items-center justify-between transition-colors duration-200")}
                    >
                      <span>{option.label}</span>
                      {count > 0 && (
                        <Badge variant="secondary" className={cn("text-xs transition-all duration-200")}>
                          {count}
                        </Badge>
                      )}
                    </Label>
          ...
          {group.type === 'color' && (
            <div className={cn("grid grid-cols-3 gap-2")}>
              {group.options.map((option) => {
                ...
                return (
                  <div key={option.id} className={cn("flex flex-col items-center space-y-1")}>
                    <Button
                      ...
                      onClick={() =>
                        handleArrayFilterChange(group.id, option.value, !isChecked)
                      }
                    />
                    <div className={cn("text-center")}>
                      <div className={cn("text-xs font-medium")}>{option.label}</div>
                      {count > 0 && (
                        <div className={cn("text-xs text-muted-foreground")}>({count})</div>
                      )}
                    </div>
                  </div>
                );
  ...
  }, [expandedGroups, filters, handlePriceRangeChange, handleArrayFilterChange, getFilterCount, toggleGroup]);

  return (
    <div className={cn("w-full max-w-sm space-y-6 pr-2")}>
      {/* Search */}
      <div className={cn("space-y-2")}>
        ...
        <div className={cn("relative")}>
          <Search className={cn("absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground transition-colors duration-200")} />
          <Input
            id="search"
            placeholder="Search..."
            value={searchValue}
            onChange={(e) => handleSearchChange(e.target.value)}
            className={cn("pl-10 transition-all duration-200 focus:ring-2 focus:ring-primary/20")}
          />
        </div>
      </div>
        ...
        <div className={cn("space-y-2")}>
          ...
          <div className={cn("flex flex-wrap gap-2")}>
            {activeFilters.map(filter => (
              <Badge key={filter.key} variant="secondary" className={cn("text-xs")}>
                {filter.label}
                <Button
                  variant="ghost"
                  size="sm"
